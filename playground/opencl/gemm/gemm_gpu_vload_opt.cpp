...
#define DType double
#define DTypeBits 64
#define KERNEL "gemm_vload_opt_fp64.cl"
#else
#define CLDType cl_float
#define DType float
#define DTypeBits 32
#define KERNEL "gemm_vload_opt_fp32.cl"
...
int main() {
    ...
    kernel = clCreateKernel(program, "gemm_vload_opt", &status);
    ...
    groups[0] = CEIL(M, BM) * BM / 4;
    groups[1] = CEIL(N, BN) * BN / 4;
                ...
                for (int ii = 0; ii < 16; ++ii) {
                    ...
                    for (int kk = 0; kk < 16 && m < M; ++kk) {
                        ...
                        for (int jj = 0; jj < 16 && kx < K; ++jj) {
                            int n = j * 16 + jj;
                            if (n < N) {
                                host_golden[m * N + n] += host_A[m * K + kx] * host_B[kx * N + n];
                            }
                        }
                    }
                }

...
#define DType double
#define DTypeBits 64
#define KERNEL "matmul_opt_fp64.cl"
#else
#define CLDType cl_float
#define DType float
#define DTypeBits 32
#define KERNEL "matmul_opt_fp32.cl"

#endif

#define PLATFORM 0
#define DEVICE 0
#define M (128*2*2*2*2*2*2)
#define N M
#define K M
...
int main() {
    ...
    kernel = clCreateKernel(program, "matmul_opt", &status);
    ...
    groups[0] = CEIL(M, BM) * BM;
    groups[1] = CEIL(N, BN) * BN;
                ...
                for (int ii = 0; ii < 16; ++ii) {
                    ...
                    for (int kk = 0; kk < 16 && m < M; ++kk) {
                        ...
                        for (int jj = 0; jj < 16 && kx < K; ++jj) {
                            int n = j * 16 + jj;
                            if (n < N) {
                                host_golden[m * N + n] += host_A[m * K + kx] * host_B[n * K + kx];
                            }
                        }
                    }
                }

			...
			for (int inc = 0; inc < num_map_input_CNN; inc++) {
				for (int wy = 0; wy < height_kernel_conv_CNN; wy++) {
					for (int wx = 0; wx < width_kernel_conv_CNN; wx++) {
						...
						tmp_weight[wx] = weight_C1[((0 * num_map_input_CNN + inc) * height_kernel_conv_CNN + wy) * width_kernel_conv_CNN + wx];
						tmp_weight[wx] = weight_C1[((1 * num_map_input_CNN + inc) * height_kernel_conv_CNN + wy) * width_kernel_conv_CNN + wx];
						tmp_weight[wx] = weight_C1[((2 * num_map_input_CNN + inc) * height_kernel_conv_CNN + wy) * width_kernel_conv_CNN + wx];
						tmp_weight[wx] = weight_C1[((3 * num_map_input_CNN + inc) * height_kernel_conv_CNN + wy) * width_kernel_conv_CNN + wx];
						tmp_weight[wx] = weight_C1[((4 * num_map_input_CNN + inc) * height_kernel_conv_CNN + wy) * width_kernel_conv_CNN + wx];
						tmp_weight[wx] = weight_C1[((5 * num_map_input_CNN + inc) * height_kernel_conv_CNN + wy) * width_kernel_conv_CNN + wx];
						__m256 weight_vec = _mm256_loadu_ps(tmp_weight);
						__m256 image_vec = _mm256_broadcast_ss(data_single_image + (inc * height_image_input_CNN + (y + wy)) * width_image_input_CNN + (x + wx));
						__m256 tmp_mul = _mm256_mul_ps(image_vec, weight_vec);
						sum_vec = _mm256_add_ps(sum_vec, tmp_mul);
					}
				}
			}
			int index = (0*height_image_C1_CNN*width_image_C1_CNN) + y*width_image_C1_CNN + x;  //当前神经元
	...
	for (int c_outer = 0; c_outer < 15; ++c_outer) {
		for (int y = 0; y < height_image_C5_CNN; y++) {
			for (int x = 0; x < width_image_C5_CNN; x++) {
				...
				for (int inc = 0; inc < num_map_S4_CNN; inc++) {
					for (int wy = 0; wy < height_kernel_conv_CNN; wy++) {
						for (int wx = 0; wx < width_kernel_conv_CNN; wx++) {
							__m256 weight_vec;
							float tmp_weight[8] = {0.0};
							__m256 image_vec = _mm256_broadcast_ss(neuron_S4 + (inc * height_image_S4_CNN + (y + wy)) * width_image_S4_CNN + (x + wx));
							for (int c_inner = 0; c_inner < 8; c_inner++) {
								int channel = c_outer * 8 + c_inner;
								tmp_weight[c_inner] = weight_C5[((channel * num_map_input_CNN + inc) * height_kernel_conv_CNN + wy) * width_kernel_conv_CNN + wx];
							}
				...
				int index = (c_outer*8*height_image_C5_CNN*width_image_C5_CNN) + y*width_image_C5_CNN + x;

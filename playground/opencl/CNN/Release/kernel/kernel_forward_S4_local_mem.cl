__kernel void kernel_forward_S4(__global float *in, __global float *weight,
  ...
  for (int ax0_ax1_fused_ax2_fused_outer_outer_outer = 0;
    ...
    data_shared[(ax0_ax1_fused_ax2_fused_outer_outer_outer)] = data[(
        (((((((int)get_group_id(2)) * 100) + (((int)get_group_id(1)) * 20)) +
           ((ax0_ax1_fused_ax2_fused_outer_outer_outer >> 1) * 10)) +
          (((int)get_group_id(0)) * 2)) +
         (ax0_ax1_fused_ax2_fused_outer_outer_outer & 1)))];
  }
  ...
  output[((((((int)get_group_id(2)) * 16) + (((int)get_group_id(1)) * 4)) +
           ((int)get_group_id(0))))] = tanh(output_local[(0)] + bias[(((int)get_group_id(2)))]);
}
